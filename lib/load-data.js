...
const load = async () => {
    ...
    const trainingFiles = Utils.shuffle(Fs.readdirSync(TRAIN_DIR).filter((name) => name.match(/\.jpg/)));
    ...
    const testFiles = Utils.shuffle(Fs.readdirSync(TEST_DIR).filter((name) => name.match(/\.jpg/)));
