...
const run = async () => {
    ...
    const train_x = tf.tensor4d(_.flatten(train.x), [train.x.length, Config.size, Config.size, 3]);
    const train_y = tf.tensor2d(train.y, [train.y.length, 1]);

    await model.fit(train_x, train_y, { epochs: 5, batchSize: 1 });

    // Evaluate model

    const test_x = tf.tensor4d(_.flatten(test.x), [test.x.length, Config.size, Config.size, 3]);
